...
export const useTableEditing = (apiUrl, state, dispatch) => {
  ...
  const handleStartEdit = useCallback((cellId) => {
  ...
  const handleSaveEdit = useCallback(async (rowId, updates) => {
    if (!apiUrl) return;
    ...
    try {
      ...
      Object.keys(updates).forEach(field => {
        dispatch({ type: ACTIONS.SET_FACETS, payload: { field, options: null } });
      });
  ...
  const fetchFacetOptions = useCallback(async (field) => {
